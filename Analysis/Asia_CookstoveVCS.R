...
cookstove_files <- list.files(path = "KMLfile/Cookstove_VCS/Asia", pattern = "\\.kml$", full.names = TRUE)
avoided_def_files <- list.files(path = "KMLfile/VCS_REDD/Asia", pattern = "\\.kml$", full.names = TRUE)
...
##########################
overlap_vcs_asia <- perform_overlap_analysis(cookstove_sf_list, avoided_def_list, "Asia", "VCS")


#### Visualize the results
if (!is.null(overlap_vcs_asia)) {
  vcs_asia_map <- visualize_overlaps(overlap_vcs_asia, "Asia", "VCS")
  print(vcs_asia_map)
}
